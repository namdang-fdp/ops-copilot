...
def process_sitemap_task(source_id: str, tech_name: str, sitemap_url: str):
    ...
    try:
        ...
        # 3. Báo cáo xong việc - Chuyển status trong DB thành ACTIVE
        if source:
            source.status = "ACTIVE"
            source.document_count = doc_count
            db.commit()

        print(f"[WORKER] Xong! Đã đẩy {doc_count} bài viết của {tech_name} lên S3.")
